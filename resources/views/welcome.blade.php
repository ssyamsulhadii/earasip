            ...
            @if (isset($has_search) && $has_search)
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-xl border border-gray-100 mt-3">
                    ...
                    <form id="search-form" method="POST" action="{{ route('upload.spk') }}"
                        ...
                        <div class="w-full md:w-auto mt-3">
                            <h3 class="mb-3">Jika tampil halaman 404 berarti SPK belum diupload
                                <a href="{{ route('lihat.spk', ['no_peserta' => $result->username, 'nik' => $result->nik]) }}"
                                    target="_blank"
                                    class="mt-4 self-end px-4 py-2 bg-yellow-600 text-white text-sm rounded-md hover:bg-green-700">
                                    <i class="fas fa-download mr-1"></i> Lihat SPK</a>
                        </div>
